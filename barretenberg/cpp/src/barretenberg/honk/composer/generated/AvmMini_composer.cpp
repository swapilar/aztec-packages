...
template <typename Flavor> void AvmMiniComposer_<Flavor>::compute_witness(CircuitConstructor& circuit)
{
    if (computed_witness) {
        return;
    }
...
AvmMiniProver_<Flavor> AvmMiniComposer_<Flavor>::create_prover(CircuitConstructor& circuit_constructor)
{
    compute_proving_key(circuit_constructor);
    compute_witness(circuit_constructor);
    compute_commitment_key(circuit_constructor.get_circuit_subgroup_size());

    AvmMiniProver_<Flavor> output_state(proving_key, commitment_key);

    return output_state;
}

template <typename Flavor>
AvmMiniVerifier_<Flavor> AvmMiniComposer_<Flavor>::create_verifier(CircuitConstructor& circuit_constructor)
{
    auto verification_key = compute_verification_key(circuit_constructor);

    AvmMiniVerifier_<Flavor> output_state(verification_key);
...
template <typename Flavor>
std::shared_ptr<typename Flavor::ProvingKey> AvmMiniComposer_<Flavor>::compute_proving_key(
...
template <typename Flavor>
std::shared_ptr<typename Flavor::VerificationKey> AvmMiniComposer_<Flavor>::compute_verification_key(
    CircuitConstructor& circuit_constructor)
{
    if (verification_key) {
...
template class AvmMiniComposer_<honk::flavor::AvmMiniFlavor>;
